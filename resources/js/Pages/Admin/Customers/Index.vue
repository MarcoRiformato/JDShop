        ...
        <div class="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-4 sm:mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3 sm:gap-4">
                <div>
                    ...
                    <p class="text-sm sm:text-base text-gray-700 mt-1">Gestisci i contatti e le richieste dei clienti</p>
                </div>
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full max-w-full overflow-hidden">
                    ...
                    <button
                        ...
                        class="flex-shrink w-full sm:w-auto inline-flex items-center justify-center px-4 sm:px-6 py-3 bg-jd-accent text-white rounded-lg hover:opacity-90 transition-all shadow-md hover:shadow-lg transform hover:scale-105 font-semibold whitespace-nowrap"
                    >
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div v-if="selectionMode" class="flex gap-2 items-center">
                        <button
                            ...
                            class="flex-shrink inline-flex items-center justify-center px-4 sm:px-6 py-3 bg-jd-accent text-white rounded-lg hover:opacity-90 transition-all shadow-md hover:shadow-lg font-semibold whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                            </svg>
                        ...
                        <button
                            ...
                            class="flex-shrink inline-flex items-center justify-center px-4 py-3 bg-jd-accent text-white rounded-lg hover:bg-jd-accent-dark transition-all shadow-md hover:shadow-lg font-semibold"
                        >
                            Annulla
                        </button>
                    </div>
            ...
            <!-- Filters -->
            <div class="bg-white rounded-lg shadow-sm border border-gray-300 p-4 mb-6">
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex-1 relative">
                        <input 
                            ...
                            class="input-jd w-full pl-10"
                        >
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-jd-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-300 overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full divide-y divide-jd">
                        <thead class="bg-gray-50 hidden md:table-header-group">
                            <tr>
                                <th v-if="selectionMode" class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider">
                                    <input 
                                        type="checkbox"
                                        @change="toggleAllCustomers"
                                        :checked="allCustomersSelected"
                                        class="h-4 w-4 text-jd-accent rounded"
                                    >
                                </th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider">
                                    Cliente
                                </th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider hidden lg:table-cell">
                                    Contatti
                                </th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider">
                                    Richieste
                                </th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider hidden xl:table-cell">
                                    GDPR
                                </th>
                                <th class="px-3 md:px-6 py-3 text-left text-xs font-medium text-jd-muted uppercase tracking-wider hidden lg:table-cell">
                                    Ultimo Contatto
                        ...
                        <tbody class="bg-white divide-y divide-jd">
                            <tr 
                                v-for="customer in customers" 
                                :key="customer.id"
                                class="hover:bg-whiteer"
                                :class="{ 'bg-gray-50': selectionMode && selectedCustomers.includes(customer.id) }"
                            ...
                            >
                                ...
                                <td class="px-3 md:px-6 py-4">
                                    <div>
                                        ...
                                        <div class="text-sm text-jd-muted">{{ customer.primary_email }}</div>
                                        <div class="md:hidden text-xs text-jd-muted mt-1">
                                ...
                                <td class="px-3 md:px-6 py-4 hidden lg:table-cell">
                                    <div class="text-sm text-gray-900">
                                        ...
                                        <div v-else class="text-jd-muted">-</div>
                                    </div>
                                </td>
                                <td class="px-3 md:px-6 py-4">
                                ...
                                <td class="px-3 md:px-6 py-4 hidden lg:table-cell text-sm text-jd-muted">
                ...
                <div v-if="customers.length === 0" class="text-center py-12">
                    <svg class="mx-auto h-12 w-12 text-jd-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                    </svg>
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Nessun cliente trovato</h3>
                    <p class="mt-1 text-sm text-jd-muted">
                    ...
                    <div v-if="!searchQuery && !gdprFilter" class="mt-6">
                        <Link 
                            :href="route('customers.create')"
                            class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-jd-accent hover:bg-jd-accent-dark"
                        >
